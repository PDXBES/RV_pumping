...
log_obj.info("STARTING PROCESS - RV Fill Fields - ".format())
...
try:
    ...
    for key, value in config.fc_field_dict.items():
        ...
        source_key_field = "FID_L0RV_pumping_sites" #comes from result of intersect #fragile - L1 refers to service index number WHICH CAN CHANGE
    ...
    with arcpy.da.UpdateCursor(config.RV_pumping_fs, ['Data_Source', 'Survey_Date']) as cursor:
        for row in cursor:
            if row[0] is None and row[1] > datetime(2022, 3, 8):
                row[0] = 'Survey'
    ...
    log_obj.info("PROCESS INTERRUPTED".format())
    arcpy.ExecuteError()
    log_obj.exception(str(sys.exc_info()[0]))

    log_obj.info("RV Fill Fields - PROCESS DID NOT COMPLETE".format())
